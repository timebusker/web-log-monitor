...
import cn.timebusker.monitor.utils.StringFilterUtil;
...
public class Log4jAsyncWriter extends Writer {
					...
					try {
						...
						for (AsyncContext ac : acQueue) {
							...
							try {
								...
								acWriter.println(appendMassge(message));
								acWriter.flush();
	...
	private static String appendMassge(String message) {
		message = StringFilterUtil.replaceBlank(message);
		StringBuilder str = new StringBuilder();
		str.append("<script type=\"text/javascript\">");
		str.append("window.parent.append(\"");
		str.append(message);
		str.append("\");</script>");
		return str.toString();
	}
